...
!  complex(kind(0d0)),PARAMETER :: ci = (0.0d0,1.0d0)
  ...
  !write(*,*) "kx=", "ky="
  !read(*,*) kx
 open(12,file='copy_BdG3KS.txt', status='replace')
...
do i=1,n
  ...
  G(4*(i-1)+1,4*(i-1)+4)=-delta
  G(4*(i-1)+2,4*(i-1)+3)=delta
  G(4*(i-1)+3,4*(i-1)+2)=conjg(delta)
  G(4*(i-1)+4,4*(i-1)+1)=-conjg(delta)
end do
do i=1,n-1

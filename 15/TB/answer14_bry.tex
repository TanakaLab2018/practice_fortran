...
\part{BdGモデルハミルトニアンについて}
	...
	\section{問題}
		...
		\begin{align}
			\mathcal{H}= \frac{1}{L_y}\int dx \sum_{k_y}
			\left[ \Psi_\uparrow \left( -\frac{\hbar^2k_y^2}{2m}-\mu_F \right)\Psi^\ast_\uparrow
			+\Psi_\uparrow \left( \Psi^\ast_\uparrow\left(x+1\right)-2\Psi^\ast_\uparrow\left(x\right)+\Psi^\ast_\uparrow\left(x-1\right)
			\right)+\Psi_\downarrow \Delta_0^\ast \Psi^\ast_\uparrow \cdots
			\right]
			\label{hamil3}
		\end{align}

...
\part{BdGモデルハミルトニアンについて}
	...
	\section{問題}
		...
		\begin{align}
			...
			+ e^{-ik_yy} \left[ -\Psi_\downarrow^\dagger\Delta_0 +\Psi_\uparrow^\dagger \left( \frac{\hbar^2}{2m}\nabla^2+\mu_F \right) \right] \Psi_\uparrow e^{ik'_yy} \nonumber\\
			+e^{-ik_yy} \left[ \Psi_\uparrow^\dagger\Delta_0 +\Psi_\downarrow^\dagger \left( \frac{\hbar^2}{2m}\nabla^2+\mu_F \right) \right] \Psi_\downarrow e^{ik'_yy}
		\end{align}
		
		よって、式\eqref{hamil1}は、
		
        %%% xの偏微分の前に\frac{\hbar^2}{2m}の係数が残ると思います．
        %%% exp(ik_yy)とexp(-ik'_yy)の積と\sigma_{k'_y}が消えるところを詳しく書いてください．
		\begin{align}
			\mathcal{H}= \frac{1}{L_y}\int dx \sum_{k_y}
			\left[ \Psi_\uparrow^\dagger \left( -\frac{\hbar^2k_y^2}{2m}-\mu_F \right)\Psi_\uparrow
			+\Psi_\uparrow^\dagger \left( \frac{\partial^2}{\partial x^2}\right)\Psi_\uparrow
			...
			+\Psi_\downarrow^\dagger \left( \frac{\partial^2}{\partial x^2} \right) \Psi_\downarrow
